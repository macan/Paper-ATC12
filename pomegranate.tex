...
\title{Evolving Directory Model, Interface and Storage Structures for Large Scale Small File Access}
...
\author{
\authname{Can Ma$\,^{1,2}$, Jin Xiong$\,^{1}$, Dan Meng$\,^{1}$, Weiping Wang$\,^{1}$, Jizhong Han$\,^{1}$}
...
%\authaddr{National Research Center for Intelligent Computing Systems,}
\authaddr{$\,^{1}$Institute of Computing Technology, CAS, Beijing, China, 100190}
\authaddr{$\,^{2}$Intern at Tencent Inc., Shenzhen, China}
} % end author
...
\begin{abstract}
    In emerging popular Web applications, e.g. online photo galleries, document and social network services, large numbers of small files are concurrently created and read. These applications exhibit low latency requirements to access small files. Moreover, \emph{N-Form} and \emph{1-N Mapping} issues of distributed file systems, that affect I/O performance of small files, have not been addressed yet.

    To boost up small file performance and address \emph{N-Form} and \emph{1-N Mapping} issues, we propose a new directory organization method for distributed file systems, which uses tabular directory model, structured storage and tabular query interface. We evaluate the new directory structures in a prototype file system PFS, and focus on improving metadata scalability elastically and exploiting locality and parallelism to improve small file I/O.

    Our evaluation shows that the new directory model, interface and storage structures can greatly increase small file I/O performance, as well as save storage space. With 32 servers, PFS delivers the leading small file I/O performance: the file creation rate was 180,000 files per seconds and the write bandwidth was 1GB/s.
\end{abstract}

%\keywords{Small Files, N-Form, 1-N Mapping, Column oriented directory}
...
\begin{figure}[t]
  ...
  \caption{Space consumption of saving 1.5 million data contents in PFS, Ext3, OrangeFS. \textit{\small By adopting CODIR, the metadata space consumption is only 1/20 comparing with PFS without CODIR. Meanwhile, CODIR uses less data and metadata space comparing with Ext3 and OrangeFS, and has the lowest latency in this test.}}
  \label{spacesavingx}
\end{figure}

    To find out whether CODIR can save metadata space for \emph{NForm} problem, we run a micro benchmark to write 1,500,000 data contents (total data size is 7.9GB) on one blade of cluster A. For PFS with CODIR, data contents are saved in \emph{three} columns of 500,000 files; for other configurations (PFS without CODIR, Ext3 with \texttt{dir\_index}\footnote{\texttt{dir\_index} option enables hashed-tree indexing for Ext3, and it speeds up the metadata performace.}, and OrangeFS), data contents are saved in 1,500,000 files. Figure \ref{spacesavingx} plots the space consumption of metadata and data and the average write latency. By adopting CODIR, PFS performs best at saving metadata cost, and has the lowest average write latency. Meanwhile, with small file aggregation, files in PFS are saved continuously without fragments. It is more space saving than block based storages such as Ext3 and OrangeFS.
...
\begin{figure}[t]
  % Requires \usepackage{graphicx}
  \includegraphics[width=.5\textwidth]{eps/ioscale.new.eps}\\
  \caption{Small File I/O Scalability in Range [1,1K]. \textit{\small Small file write rates increase linearly with more clients until servers are saturated.}}\label{ioscale}
\end{figure}

    Firstly, we want to find out the scalability of small file aggregation. We ran \texttt{cluster postmark} in cluster A to investigate the scalability of small file (in range [1,1K]B) I/O. In each run, we fixed the number of servers, and variate the number of clients to change loads. Figure \ref{ioscale} plots file write rates varying with different number of clients and servers. When there were fewer servers, e.g. the \texttt{\#Server=1} line, system saturates quickly and the file write rate is not linearly increasing. When there are more servers, e.g. the \texttt{\#Server=32} line, file write rate keeps increasing linearly.
...
\begin{figure}[t]
  ...
  \caption{Small File Create Rates and Write Bandwidth. \textit{\small For OrangeFS, clients totally creates 4.7 million files; while for PFS, clients totally creates 47 million files. With increasing the file size, file create rate decreases. The aggregated write bandwidth of PFS with 512KB file is more than 1GB/s.}}\label{smallcreates}
\end{figure}

    Secondly, we want to know whether file aggregation is effective for concurrent small file creations. We ran \texttt{cluster postmark} (47 processes) in cluster A with file size from 1KB to 512KB, and compared file create rates and bandwidth between PFS and OrangeFS. Both PFS and OrangeFS were configured with 32 metadata and data servers. Figure \ref{smallcreates} plots the aggregated create rates and write bandwidth. File create rate and write bandwidth of PFS is more than 6000\% of OrangeFS. By enlarging the file size, file create rates of both file systems are decreasing, while write bandwidth are increasing. OrangeFS performs worse when small files are larger than 64KB. While, the aggregated write bandwidth of PFS with 512KB files is more than 1GB/s. This bandwidth promotion benefits from our scatter and aggregation small I/O flows in Figure \ref{io_write_flow}.
...
\begin{figure}[t]
  ...
  \caption{Small File Read/Append Bandwidth. \textit{\small File read and append bandwidth of PFS is larger than that of OrangeFS. The random read bandwidth of PFS drops at 8KB because the network MTU is only 5100B. Both read and append bandwidth drops at 16KB because PFS client uses 16KB buffer page. Thus files large than 16KB have to load and write more pages, and it means longer latency. The RMW issue in file append has little impaction on I/O bandwidth.}}\label{smalliora}
\end{figure}

    Next, we want to know how RMW issue affect small file I/O performance. We ran \texttt{cluster postmark} (75 processes) in cluster B to randomly read or append files (1:1) with file size from 512B to 2MB on PFS and OrangeFS. Both PFS and OrangeFS were configured with 75 metadata and data servers\footnote{OrangeFS failed to run cluster postmark when files are larger than 32KB. This might be a bug of multi-metadata server implementation.}, and mounted with \texttt{-noatime} option. Figure \ref{smalliora}\footnote{The aggregated network bandwidth of cluster B is about 200MB/s, and it is mainly limited by the top-level GigEther switch.} plots the I/O bandwidth of file read and append. Both read and append bandwidth of PFS increase with larger file size, and outperform that of OrangeFS. Although there are RMW issues in file append, the append bandwidth increases steadily to the maximum network bandwidth.
...
\begin{figure}[t]
  ...
  \caption{Runtime and Space Consumption of import 1 million Gene Sequences. \textit{\small Import short gene sequences into PFS is fastest and most space efficient with respect to OrangeFS and Ceph. This test runs on one machine.}}
  \label{spacesavingy}
\end{figure}

    Finally, we ran a real world gene sequencing program to import 1 million short gene sequence files (most $<$ 1KB) into PFS, OrangeFS and Ceph. Ceph is configured to store one data and metadata replica\footnote{We had tried to tune many different configuration variables, Ceph (with FUSE client) ran slowly still and finished the test in 3 days.}. Figure \ref{spacesavingy} presents the total space consumption and runtime. Benefit from small file aggregation, PFS is much more space efficient and faster than Ceph and OrangeFS. It is \emph{10 times} faster than OrangeFS, and the image size is only 19.7\% of OrangeFS and 7.6\% of Ceph.
...
\begin{figure}[t]
  ...
  \caption{Tag Set and Search Latency. \textit{\small Using sorted file $+$ tabular query interface, tag set and search are faster than external database(BDB) and brute-force scanning search. Especially, external database approach is slow at importing tags into database. And for queries of lower selectivity, sorted file $+$ tabular query performs much more better.}}\label{xattr}
\end{figure}

    In this test, we set up 1 million files and gave each file a specific tag. For CODIR model, file tags were saved in sorted file; for traditional directory model, file tags were saved in extended attributes; while for external database model, file tags were saved in external BerkeleyDB\footnote{The database is configured \emph{without} transaction support, and in Concurrent Data Store mode (CDB).}. We ran queries through CODIR's tabular query interface, and compared it with brute-force scanning and external database. The first two tests ran through \texttt{xattr} syscall interface. Figure \ref{xattr} plots tag set and search latency for different selectivity. Comparing with external BDB, sorted file decreases both tag set and search latency, because it is compact and tightly coupled with file system. Comparing with brute-force scanning, sorted file and database based searches performs better. Their latencies are sensitive to selectivity. For queries with lower selectivity, the latency is lower, because more files are ignored to stat.

    \textbf{Lesson \#5:} Searches based on sorted file with tabular query interface performs better than external database indexing and brute-force directory scanning.

\subsection{Elastic Metadata Service}

    Firstly, we want to find out whether PFS could scale out. We used \texttt{mdtest} to evaluate metadata performance of PFS, GIGA+, OrangeFS, Ceph, and HBase in cluster A. Our configuration used 8 processes per client to simultaneously create files in a shared directory, and the number of files created is proportional to the number of servers: 12.8 million files on 32 servers. HBase is used to simulate the metadata service of Google's Colossus file system\cite{gfs}. PFS and OrangeFS use FUSE client to complete the test. Each file \texttt{creat()} in PFS results in two RPC calls to server: \texttt{getattr()} to check if the file exists, and \texttt{creat()} to create and return file's attributes.
...
\end{figure}
    ...
    We reuse the experiment results of GIGA+, Ceph, and HBase from the original paper\cite{giga,ceph} that performed on different clusters. Those clusters used quad-core 2.83GHz machines with 10GigE NICs and switches (faster than ours). Figure \ref{mdscale} compares the aggregated file create rates, in file creates/second. PFS scales \emph{linearly} up to the size of 32-servers configuration, and sustains 180,000 file creates per second. PFS also demonstrated scalable performance for concurrent lookup workload and delivered a throughput of more than 410,000 file lookups/second (not shown). It benefits from Two-Stage slice split strategy and removing out-of-core indexing. Although OrangeFS (experimental version) had implemented GIGA+ like distributed directories, we found that it performs worse (the bottom line) than GIGA+.
...
\end{figure}
    ...
    Next, we want to find out the metadata behavior of Two-Stage slice split strategy (firstly split to remote servers, then split at the local servers). The test ran on cluster B with 75 metadata servers and clients. Clients issued millions of file create requests. Figure \ref{splits} plots instant request rate and numbers of two types of splits in three runs with local only, random, and Two-Stage split strategies. It is obviously that Two-Stage splits are far more better than local only and random splits. After entering into local split mode (after 40s), serviced request rate increases for about \emph{10 times} larger\footnote{To plot the two stage clearly, we slow the transition from remote splits to local splits in Two-Stage strategy intentionally.}.
...
\begin{figure}[t]
  ...
  \caption{Cumulative Distribution Function of latency. \textit{\small Online a server shifts the curve to lower latency(Conf A to B). Offline a server shifts the curve to higher latency(Conf B to C). It means that PFS scales up and down elastically.}}\label{onlineoffline}
\end{figure}

    Finally, we want to find out whether the metadata service of PFS could scale up and down elastically. To understand how server changes impact on the latency of file operations, we ran a test to do file creates, lookups, and removes in three configuration. \emph{Conf A} ran the test on \emph{one} server without any online/offline operation. In \emph{Conf B}, we initially ran on one server, and online a new server in the test. In \emph{Conf C}, we ran on two servers, and offline a server in the test. Figure \ref{onlineoffline} plots the cumulative distribution function of request latency in three configurations. Adding a server makes average latency smaller (\emph{Conf A} to \emph{B}), while deleting a server makes average latency larger (\emph{Conf B} to \emph{C}). The metadata service performs elastically when changing servers.

\subsection{Relayed Large File I/O}

\begin{figure}[t]
  % Requires \usepackage{graphicx}
  \includegraphics[width=.5\textwidth]{eps/largeio.new.eps}\\
  \caption{Large File I/O Bandwidth in large cluster. \textit{\small Comparing to SkyFS with 30 storage nodes, PFS performs better. It indicates that optimizations for small files do not affect large file I/O.}}\label{largeio}
\end{figure}

    Finally, we want to know whether our optimizations for small files has affected large file I/O performance. To understand the tradeoffs of our design choices on large file I/O, we ran \texttt{IOZone} on cluster A to investigate. In this test, we used SkyFS (with 30 nodes) as PFS's large file store, and compared PFS with native SkyFS. For PFS, all I/O requests are routed to SkyFS data servers. \texttt{IOZone} is configured with 1MB record size, and the total file size is \emph{three} times larger than memory size. Shown by Figure \ref{largeio}, comparing with SkyFS, PFS performs a little better. Because SkyFS also uses FUSE client, we consider that this strange behavior might be some characters of FUSE.

    \textbf{Lesson \#8:} Even considering the FUSE overhead, large file I/O in PFS has not gone bad in large cluster.

\section{Conclusion and Future Work}

    In this paper we address the emerging requirement for large scale small file access. We focus on two common problems: \emph{N-Form} and \emph{1-N Mapping} in distributed file systems, and evolve directory model, interface and storage structures for small files. Firstly, we propose to use tabular directory model and interface to not only save lots of spaces, but also increase the efficiency of metadata service. On the other hand, we propose to use directory based small file scatter and aggregation to improve small file I/O performance. Our evaluation shows that by using CODIR model, interface, and storage structures, file system achieves best-in-class performance on commodity clusters. Especially, metadata and small file performance scale linearly with more nodes.

    PFS is released at \texttt{github.com} for researchers who want to test, tune or integrate it. In the future, we will extend POSIX interface for large scale small file access.
...
\bibitem{facebook-haystack} Beaver, Doug and Kumar, Sanjeev and Li, Harry C., et al, \emph{inding a needle in Haystack: facebook's photo storage}. Proceedings of the 9th USENIX conference on Operating systems design and implementation (OSDI 2010).

\bibitem{taobao-tfs} Wensong Zhang, \emph{Taobao File System}, \url{http://rdc.taobao.com/blog/cs/?p=128}.

\bibitem{trf} Rongfeng Tang, Private Communication, (2011).

\bibitem{bigtable} Chang, Fay and Dean, Jeffrey and Ghemawat, Sanjay, et al, \emph{Bigtable: a distributed storage system for structured data}, Proceedings of the 7th USENIX Symposium on Operating Systems Design and Implementation, (OSDI 2006)

\bibitem{cstore} Stonebraker, Mike and Abadi, Daniel J. and Batkin, Adam, et al, \emph{C-store: a column-oriented DBMS}, Proceedings of the 31st International conference on Very Large Data Bases, (VLDB 2005)

\bibitem{cassandra} Lakshman, Avinash and Malik, Prashant, \emph{Cassandra: a decentralized structured storage system}, SIGOPS Operating System Review vol 44, (2010)

\bibitem{ycsb} Cooper, Brian F. and Silberstein, Adam and Tam, Erwin, et al, \emph{Benchmarking cloud serving systems with YCSB}, Proceedings of the 1st ACM symposium on Cloud computing, (SoCC '10)
...
\bibitem{ch} Karger, D., Lehman, E., Leighton, T., et al, \emph{Consistent Hashing and Random Trees: Distributed Caching Protocols for Relieving Hot Spots on the World Wide Web}, Proceedings of the 29th ACM Symposium on Theory of Computing, (1997)

\bibitem{reiserfs} Hans Reiser. ReiserFS. \url{www.namesys.com}.
...
\bibitem{hfs} Zhang, Zhihui and Ghose, Kanad, \emph{hFS: a hybrid file system prototype for improving small file and metadata performance}, Proceedings of the 2nd ACM SIGOPS/EuroSys European Conference on Computer Systems, (EuroSys 2007)

\bibitem{giga} Swapnil Patil, Garth Gibson, \emph{Scale and concurrency of GIGA+: file system directories with millions of files}, Proceedings of the 9th USENIX conference on File and stroage technologies, (FAST'11)
...
\bibitem{fuse} FUSE, Filesystem in Userspace, \url{fuse.sf.net}.
...
\bibitem{gfs} McKusick, Marshall Kirk and Quinlan, Sean, \emph{GFS: Evolution on Fast-forward}, ACM Queue, (2009)
...
\bibitem{smartstore} Yu Hua, Hong Jiang, Yifeng Zhu, Dan Feng and Lei Tian. \emph{SmartStore: A New Metadata Organization Paradigm with Semantic-Awareness for Next-Generation File Systems}, Proceedings of the International Conference for High Performance Computing, Network, Storage and Analysis, (SC 2009)

\bibitem{lustre} P. Schwan, \emph{Lustre: building a file system for 1000-node clusters}, in Linux Symposium, (2003).

\bibitem{pvfs} Ligon, III, W.B., and Ross, R. B., \emph{PVFS: Parallel Virtual File System}, Beowulf Cluster Computing with Linux, pages 391-430, MIT Press, (2001).
...
\bibitem{ipdps09} Philip Ross Carns, Sam  Lang, Robert Ross Kunkel, Thomas  Ludwig, \emph{Small File Accesses in Parallel File Systems}, Proceedings of the International Symposium on Para. and Dist. Proc., (IPDPS 2009)
