...
\subsection{Tabular Directory Model}
    ...
    Typically, current file system directory is a plain (key/value) table with three columns: \emph{name}, \emph{metadata}(including extended attributes, abbr. xattr), and \emph{data}. Naturally, for a photo with several sized images, each image is saved in a standalone file, shown by Figure \ref{directory_model}(a). Obviously, we have \emph{N} copies of file metadata. Alternatively, we can group these \emph{N} images together by xattr and self-packing to eliminate redundant metadata and boost up metadata accessing. However, it is non-trivial to store several image files with one file name in plain directory model. Firstly, it is impossible to save large data content in xattr. Linux VFS \texttt{xattr} only supports setting and getting at most 64KB buffer. Even worse, local file system, e.g. Ext3, sets up a smaller buffer limit at 4KB. Secondly, user has to pack multiple image files into one file's data content and record offset and length of each image in xattr, shown in Figure \ref{directory_model}(b). Although it is possible to save \emph{N} image files under one name, this approach is inconvenient for users. Other applications have to build their own packing and unpacking framework.

\begin{figure}
  % Requires \usepackage{graphicx}
  \includegraphics[width=.5\textwidth]{directory_model.eps}
  \caption{Directory Model for N-Form Problem. \textit{\small Imagea-s and imagea-m are the small and medium forms of imagea. Plain model is either (a) metadata costing or (b) difficult to use. Column oriented tabular model resolves both these difficulties.}}\label{directory_model}
\end{figure}
    ...
    In CODIR's table, there might be unlimited columns. If columns' metadata are saved in dentries, then dentries are variably sized. Directory operations such as \texttt{seekdir} would be difficult to implement. Thus, we make dentries have a fixed size. Each dentry contains fixed number of \emph{direct} columns and unlimited \emph{indirect} columns. The frequently accessed data should be put in the direct columns, while the infrequently accessed ones should be put in the indirect columns.

    As directory contains lots of columns, it might be sparse. Some dentries do not have valid values for some columns. Obviously, these blank cells do not need to be saved. For each column, valid data are sent to storage server and aggregated in a column file, shown by Figure \ref{storage_engine_model}. The data written offsets must be kept in dentries (as metadata) for read and write in the future. Data offsets of direct columns are stored in dentries, while data offsets of indirect columns are stored in an \emph{indirect file}. Thus, accessing data of direct columns are faster than that of indirect columns. Besides, if the table is sparse, it is space consumptive for indirect columns to keep these offsets in a fixed sized array with holes. Thus, the indirect file is append-only with a tail index record. For each update, the update offset and an index record (indicates column ranges and internal offsets) are appending to tail of the file. On reading, the index record is read in firstly to determine which file position should be read next.

    Although the tabular directory supports sparse table, we argue that sparse tables decrease directory performance and waste more space. To eliminate sparse tables in practice, users should only group files with similar properties in one directory. For example, do not mix user photos and documents together in a directory. Fortunately, this simple rule always keeps because of our habit to classify things before storing.
    ...
    The tabular directory model can save DFS's metadata for \emph{N-Form} problem. Other than metadata saving, this model has some other advantages. Firstly, it is more efficient to store larger data than extended attributes. In many implementations, \texttt{xattr} attaches to file metadata, and is not very efficient at storing large data content. While, CODIR permits to store several variable length of data in it. And these data are not required to store with metadata. Secondly, it is more efficient to do data compression on columns. Shown by Figure \ref{storage_engine_model}, we aggregate one or several columns' content (e.g. \emph{Metadata}) to one storage file. Thus, for each directory, we have already grouped similar data together. It is efficient for compressor to get a better zip ratio. For example, we had compressed the \emph{Metadata} file for about \emph{25 times} smaller by using LZO\cite{lzo} in PFS. Thirdly, it is more efficient to do bulk load on specific columns. For example, file content of the same column are stored in the same storage file, thus, it is suit for bulk loading. This is valuable for Web applications that needs bulk file loads in the cache filling stage. For example, in photo galleries, loading small sized images (aka thumbnails) to cache are very fast with CODIR, because these are all sequential I/Os.
...
\end{figure}
    ...
    Current distributed file systems mostly map their files to files of local file system on storage nodes. Thus, even you resolve the \emph{N-Form} problem, you still have the \emph{1-N Mapping} problem that increases small file access latency. Shown by Figure \ref{storage_engine_model}, to reduce this storage file metadata lookup latency, we propose a directory-based file aggregation approach. This aggregation reduces metadata, and is not only saving disk space but also saving memory (no need to cache lots of dentires of local file system). Small files in one directory are logically grouped in one big storage file to eliminate redundant metadata. Each data access is actually a read or write in this big file. Thus, ideally, for each directory, we only have to lookup and open one storage file. By caching this file's metadata, data I/O of small files does not need to do metadata lookups in local file system thereafter. Meanwhile, a file system directory is naturally a locality set. Files in one directory might be accessed contiguously. Combining files of the same directory together not only doesn't ruin, but also improve the spatial locality.

    At the same time, in CODIR, data contents are aggregated by column by default. The rationale of splitting one file's columns into several storage files is to reduce unnecessary data loading in column fetching (and prefetching). For example, in a photo gallery, when you visit a list of previews, it is unnecessary and time consuming to read in every photo's original file. However, co-accessed columns could be grouped and stored together. For example, in Figure \ref{storage_engine_model}, we group file's metadata and data content together. Each file lookup, that read-in the metadata, would prefetch the data content to cache either. Subsequent data access would hit in the cache.

    \subsubsection{Aggregated Storage Files}

    We use log structure storage format for the aggregated file data. For small files, it is more space efficient than block based storage format because of no internal fragments\cite{lfs}. New arriving data is appended to tail of the file. Removing files are only metadata operations and do not need to update data file. Every small file write must contain the full file content. Thus, file rewrite has to be transformed to read-modify-write (RMW).

    After lots of file deletions or modifications, the aggregated data file might be filled with holes. These holes must be detected and garbage collected to save storage space. For each directory, metadata that needed in garbage collecting (GC) co-locates with the data. Thus, the GC process runs incrementally against directories on each storage node to not interfere with storage process.

    The RMW issue of file rewritten might be a performance bottleneck. However, in Web applications, there are fewer file rewritten than file creations and reads. Further more, \texttt{a)} read small file (in stage R) means less time to stall, and \texttt{b)} lots of rewritten are actually issued after file reads (which matches the order of stages R and M). Thus, we argue that RMW issue would has more impacts on larger files than smaller ones and it is worth sacrificing rewrite performance to optimize file creation and read.

    \subsubsection{Structured Storage Files}

    Dentries are stored in an aggregated storage file: \emph{Metadata}. Number of dentries grouped as a \emph{slice}, which is the basic metadata read and write unit. Slice is different from directory partition in GIGA+\cite{giga} and SkyFS\cite{skyfs}. Slice uses hash table indexing, while directory partition uses out-of-core indexing. It reduces access latency for small files by eliminating out-of-core indexing.

    It is slow to do slice lookups in the log structure \emph{Metadata} file. Thus, an index file is built, \emph{Boost File}, for the metadata file to speed up lookups. The index keeps a record $<$slice, offset$>$ for each active slice. The \emph{Boost File} is a supplement of the \emph{Metadata} file. It is guaranteed to be updated on every slice write. Even if corrupted, it can be regenerated by scanning the \emph{Metadata} file. Slice in \emph{Metadata} file has a header, which contains magic numbers and special inequations. These can be used as border markers of slice in scaning.

\begin{figure}
  % Requires \usepackage{graphicx}
  \includegraphics[width=.5\textwidth]{index_mapping.eps}
  \caption{Sorted File Format. \textit{\small Columns of directory X and Y are directed to Sorted File $\alpha$. Each database contains one base file and several secondary indexes.}}\label{index_mapping}
\end{figure}

    Semantic searches, e.g. ``\textit{Top-K query: find the K hot jpeg files}'', are widely used in many applications\cite{spyglass,smartstore}. In CODIR, columns could be used to store application specific tags or xattrs. These tags and xattrs are used to serve semantic searches. Researches\cite{spyglass,smartstore} have found that using an external database ruins the spatial query locality, and proposed many semantic search approaches on large scale file systems. However, most of these approaches depend on offline file scanning, and are not capable to handle mutable file systems (for Web2.0 applications). To support online indexing, we propose to \textbf{1)} use internal \emph{sorted} file instead of external database, \textbf{2)} use Star schema to maintain relationships among columns, and \textbf{3)} only manage user-defined subset of directory columns to decrease file subset size that we handle. At the same time, sorted file is tightly coupled with file system to achieve lower latency. For example, shown by Figure \ref{index_mapping}, columns \emph{A, B} from directory \emph{X}, and columns \emph{C, D, E} from directory \emph{Y} are kept in one sorted file \emph{$\alpha$}. Each sorted file contains a base file to save basic mappings from file name to unique file id(File$|$ID), and several secondary indexes for each column(e.g. A$|$ID). In this way, it is easy to dynamically add new column or delete old column. In all, this approach not only maintains spatial locality, but also provides more flexibility. The sorted files could be key/value stores such as BerkeleyDB\cite{berkeleydb}. Thus, in Figure \ref{storage_engine_model}, query ``\textit{find all the jpeg files}'' can be executed by searching the \emph{sorted} file and locating file \emph{imageb} directly, rather than scanning the whole directory to find the file.
    % specify the BDB configurations

\subsection{Tabular Query Interface}
\label{interface}

    % exploit the existing xattr interface: getxattr, setxattr, listxattr, ...
    Firstly, as we have extended the directory model to store multiple columns, we need an interface to get/set table cells. Secondly, in large scale file system, to find a file subset, searching is more meaningful than traversing. As a result, we provide a user level tabular maintain and search interface, and also wrapped it to fit the existing \texttt{xattr} interface\footnote{Note that, it is impossible to do large data read/write with this wrapper because of the 64KB buffer limit in Linux VFS.}: \texttt{getxattr} and \texttt{setxattr}.
    ...
    \begin{tabular}{c|l}
      ...
      & \texttt{\scriptsize (filepath, EMBEDDED\_CMD\_STRING, buf,len)}\\
      \hline
      ...
      \hline
    \end{tabular}
    \caption{User Level Interface for File and Directory}
    \label{tabular_interface_table}
    \end{table}

    As shown by Table \ref{tabular_interface_table}, we defined three API groups for CODIR. The \textbf{table} group is for applications. When opening the directory table, user can read/write specific file column and do bulk load on columns.
    The \textbf{raw} group is for library developers. They can read/write specific file column by column ID instead of name directly. The \textbf{sorted} group is for tag operations. A list of key/value pairs can be attached with each file. KV pairs from specific columns of directories are grouped in one sorted file. These pairs are automatically indexed. By using sorted file, queries and joins on file tags are more quickly than brute-force scanning the whole directory tree.

    The \texttt{xattr} wrappers encode tabular operations in strings. For example, string ``pfs.sorted.set.S.file.k1=v1, k2=v2'' represents a tuple ($<$file, $k1=v1;k2=v2$$>$) addition on sorted file \texttt{S}. String ``pfs.sorted.search.S.R: $k1<10\&k2>20$'' represents a range (\texttt{R:}) search on sorted file \texttt{S} that satisfies $k1<10\&k2>20$. The search operation returns an iterator, which allows you iterate the result set.
...
\end{figure}
    ...
    We have implemented the tabular directory model, storage structures, and interface in a prototype distributed file system PFS for Web applications that need to access large scale small files. Large files are handled by other DFSs. Shown by Figure \ref{hvfs_architecture}, PFS contains a FUSE\cite{fuse} client, a dedicated metadata and storage cluster to handle small files, and relies on Ext3, SkyFS (or Ceph, PVFS2) to handle large files. However, CODIR might be implemented in other DFSs to improve their small file performance.
    ...
    The storage cluster is a shared storage pool. Metadata and data of small files are aggregated by directory and scattered to all the servers. Thus, for each directory, there are several sequential data write flows to exhaust large scale concurrent file creations.

\subsection{Distinguish Small Files}
    % static or dynamic
    Small files are typically less than 1MB\cite{gfs}. In many photo galleries, we found that the average file size is at 32KB. Thus, for higher degree of confidence, we make 2MB as the transition point from small file to large file. But, at file \texttt{open} time, there is no hint on file length. There are two approaches to distinguish small files. The first one is a \emph{static} approach. The parent directory is marked as \emph{static-small} and all file creations in it automatically inherit the property. The second one is a \emph{dynamic} approach. The parent directory is marked as \emph{dynamic-small}. Static-small doesn't allow a small file to transform to a large one automatically, while dynamic-small allows that.

    At \texttt{open} time, we use parent directory's property to guide following I/O operations. When it is static-small, the followings are all small file operations. When it is none, the following are all large file operations. When it is dynamic-small, the PFS client probes on each file write and determine if the file is larger than 2MB to be able to change from small file to large file or vice verse.

\subsection{Elastic Metadata Service}

    Metadata service is the bottleneck in many distributed file systems\cite{lustre,gfs}, especially for small files. Even in scalable file systems\cite{ceph,giga,pvfs}, metadata performance has been slowed down by storage structures(e.g. out-of-core indexing). Thus, CODIR and file aggregation, that decrease and aggregate metadata, could increase metadata performance either. Meanwhile, for large scale small file creations, metadata service has to be able to scale out to large number of machines. But, in practice, it is difficult to estimate how many machines are enough for user requests before starting up. Thus, it requires \emph{elastic} metadata service which supports adding or removing machines online.

    % scale out
    To scale out to a large cluster and keep low latency, the metadata service of PFS uses extendible hashing\cite{giga} to manage slices of directory tables. With lots of file creations, slice splits to store more files. The new slice might be managed on the same metadata server (aka. local split) or other remote servers (aka. remote split). If the new slice is placed on remote servers, more server processing capability might be used. However, there always exists data transfer overhead in remote splits. Thus, in PFS, we make slices firstly split to remote servers to exploit more machines, and secondly split at the same server to eliminate data transfer overhead(named Two-Stage split).

    % elastic
    To scale elastically, PFS uses consistent hashing to distributed table slices to servers. Thus, when adding or removing machines, slices that need to be transferred to remote machines are least\cite{ch}. At the same time, because of metadata service of PFS is a caching service, slices that need to be transferred could be simply dropped at original server and reload on demand by the new server. This approach mitigates data transfers on machine changes, and results in lower service interruption.

\subsection{Parallel Small File I/O}

    In practice, applications are expected to use one directory to store one type of files as far as possible. Thus, PFS storage cluster aggregates small files based on directory. I/O operations for small files are first scattered to a list of servers, and then aggregated by directory. PFS exploits both spatial and temporal locality to boost up small file I/O performance.
...
\end{figure}
    ...
    Each server opens one aggregated file for a directory. Instead of in-place modification, file creations and writes are appended to the file. If there are lots of file creations, the creation flow is directed to the file to exploit sequential I/O bandwidth. The temporal continuous data flows (split by column) are transformed to spatial continuous data streams. Further more, because of file creations in one directory are scattered to several servers, actually PFS sets up several sequential I/O flows to exhaust lots of concurrent file creations, shown by Figure \ref{io_write_flow}. Note that, every file written must be an integrated writes.

    At the same time, columns are stored in separated storage files by default. Thus, each file has already aggregated contents that has similar property. For example, in Figure \ref{storage_engine_model}, storage file for column `Medium' contains all the files of medium size. For application requests that need to scan all medium images (violence detection), it is efficient to do sequential reads on the storage file. As columnar store in database, this exploits the vertical spatial locality in directory table.
